...
{
    uint8_t value;
    HAL_I2C_Mem_Read(sensor->i2c_handle, sensor->address, reg, 1, &value, 1, 1000);
    return value;
...
uint16_t TSL2591_Read_Word(TSL2591_t *sensor, uint8_t reg)
{
    uint8_t buffer[2];
    uint16_t value;
    HAL_I2C_Mem_Read(sensor->i2c_handle, sensor->address, reg, 1, buffer, 2, 1000);
...
void TSL2591_Enable(TSL2591_t *sensor) {
    TSL2591_Write_Byte(sensor, COMMAND_BIT | ENABLE_REGISTER, ENABLE_POWERON | ENABLE_AEN);
}
/******************************************************************************
...
void TSL2591_Disable(TSL2591_t *sensor) {
    TSL2591_Write_Byte(sensor, COMMAND_BIT | ENABLE_REGISTER, ENABLE_POWEROFF);
}
/******************************************************************************
...
Info:
******************************************************************************/
UBYTE TSL2591_Get_IntegralTime(TSL2591_t *sensor)
{
    UBYTE control=0;
    /************************************************************
...
Info:
******************************************************************************/
UWORD TSL2591_Read_Channel0(TSL2591_t *sensor)
{
    return TSL2591_Read_Word(sensor, CHAN0_LOW);
}

UWORD TSL2591_Read_Channel1(TSL2591_t *sensor)
{
...
Info:
******************************************************************************/
void TSL2591_Init(TSL2591_t *sensor, I2C_HandleTypeDef *i2c, uint8_t address) {
...
******************************************************************************/
void TSL2591_SET_InterruptThreshold(TSL2591_t *sensor, UWORD SET_LOW, UWORD SET_HIGH) {
	TSL2591_Enable(sensor);
...
******************************************************************************/  
void TSL2591_SET_LuxInterrupt(TSL2591_t *sensor, UWORD SET_LOW, UWORD SET_HIGH)
{
    double Cpl;
    double again;
